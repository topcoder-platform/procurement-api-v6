...
The [mirror workflow](../.github/workflows/codecommit-mirror.yml) maintains an exact secondary Git copy of `topcoder-platform/procurement-api-v6` in the dedicated `procurement-api-v6` AWS CodeCommit repository.
...
- CodeCommit repository: `procurement-api-v6`
- CodeCommit default branch: `develop`
- IAM role: `GitHubActions-procurement-api-v6-CodeCommitMirror`
...
The `GitHubActions-procurement-api-v6-CodeCommitMirror` trust policy accepts only OIDC subjects for this repository's `develop` and `master` branches. It accepts both GitHub's legacy name-based subject and the repository's immutable owner/repository-ID subject. Public forks and workflows on any other upstream ref have different subjects and are denied.
...
Those permissions apply only to `arn:aws:codecommit:us-east-1:811668436784:procurement-api-v6`. The role cannot access other CodeCommit repositories, delete repositories, manage IAM, or use other AWS services.
